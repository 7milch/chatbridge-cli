...
- `send(text)` while the status is `busy`, `opening` or `reopening`
  **queues** instead of refusing; while `dead` it starts a turn (lazy reopen)
  unless entries are already queued, in which case it queues behind them. The pending attachments go into the queue
...
- **Up** in an empty composer with a non-empty queue posts `takeBack`; the
  host answers with `tookBack` (the entries it removed) and then a `state`;
  the webview fills the composer from `tookBack` only, so an entry drained
  in between is never re-sent. Attachments that no longer fit under the
  total cap are left out and a warning says how many.
...
### TUI starts before the browser

Today `runInteractive` opens the `ChatSession` before any UI exists, so an
unauthenticated start exits 2 with the `auth login` hint. With `/login`
available inside the TUI that ordering inverts, like the VSCode sidebar:

- The renderer and the view come up first, with the model in a new status
  `opening` and the status row `Opening browser...`. `ChatModel` no longer
  takes a session in its constructor; it starts with `current` undefined
  and calls `openSession()` itself, eagerly, right after construction (so
  the first message is not delayed by a lazy open).
- Input typed while `opening` is queued and drains once the session is
...
- Progress messages from the open are shown by the status row (the
  stderr buffering in `runInteractive` is unchanged).
- Teardown: `settleReset` also covers the initial open, so quitting while
  `opening` waits (capped) for the session to exist before closing it.
- `runtime-check` (Bun / Node version) still runs before anything, as it
  decides whether OpenTUI can load at all.
...
- `chat-model.test.ts`: constructor opens eagerly, queue drains after the
  initial open, failed open → dead with the `/login` hint; each slash command, `/login` cancel / success /
