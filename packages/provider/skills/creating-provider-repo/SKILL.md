...
later, use the upgrading-provider-repo skill.
...
- rename `gitignore` → `.gitignore`, `mcp.json` → `.mcp.json`, and under
...
src/provider.e2e.test.ts  real service; needs <VENDOR>_E2E=1 and saved auth state
docs/dom-notes.md  eight sections, filled by dom-discovery.md
README.md  CLAUDE.md  vscode/ (optional)
```
...
4. **Verify by hand**: `--version` → `auth login` → `-p "Reply with the
   single word: ping"` → the gated E2E → interactive mode → `auth logout` /
   `auth status`.
...
What `templates/src/provider.ts` implements; read when debugging a VARIANT.

| Method | Must |
|---|---|
| `navigateToLogin` | `goto` the entry URL; the user completes login by hand. |
| `isLoggedIn` | Polled every second during `auth login`, once at startup. (1) origin differs from `chatUrl`'s → `false`, DOM untouched (the IdP page; a no-op when login is same-origin); (2) in one `try`, wait ≤10 s for the sign-in or the account control to be **visible**; (3) account present AND sign-in absent; (4) `catch` → `false`. Visible-only locators: a hidden twin times the wait out on every poll. |
| `startNewChat` | Navigate or click, then wait for the composer to be visible and empty. |
| `sendMessage` | Record the assistant-message count (module-level `WeakMap<Page, number>`), fill (works on `contenteditable` too), submit, then wait briefly for the generating state to begin (ignore timeout). |
| `waitForResponse` | Done signal, then the count exceeding the recorded one, then the newest message read until two reads 500 ms apart agree. Never an earlier turn. Done signal first: a placeholder turn may appear and be removed before the real one. |
...
| `commands` (≥ 0.9.0) | `/name` commands (TUI and VSCode): `{ name, description, run(page, args) }` → `{ kind: "show", text }` or `{ kind: "send", prompt }`. Lower-case names, never a built-in, not in one-shot mode. |
| `urlHooks` (≥ 0.9.0) | `{ match, resolve(url) => { label, content } }`. Fetching is the provider's; the framework appends the content as an attachment, under the session timeout and the `MAX_*_BYTES` caps. Trailing prose punctuation is trimmed first. |
...
- `browser: { reducedMotion }` — leave it at `"reduce"`: an animating idle page
  burns CPU all session. Write `waitForResponse` against DOM state, never an
  animation.
- `idle: { timeoutMs }` — default idle lifetime of an interactive session (24 h;
  ...
  `default`: a declared default silently overrides the provider's value; put
  the fallback in the `description`.
...
| `auth login` saved a state that restores as logged out | Session in IndexedDB, or state captured on the IdP origin | Framework ≥ 0.2.0 saves IndexedDB; `isLoggedIn` is false off-origin |
...
| `waitForResponse` returns "…" or an ellipsis | A placeholder assistant element appears, is removed, then the real one is inserted | Decision table |
| Page title "Just a moment...", `isLoggedIn` false | Cloudflare challenge in headless Chromium | `detectBlock` → the CLI exits 6 and says `Try --headful`; evasion is out of scope |
| An IdP refuses the automated browser even headful | It fingerprints the browser | Log in with a password or emailed code; note it in §Login |
